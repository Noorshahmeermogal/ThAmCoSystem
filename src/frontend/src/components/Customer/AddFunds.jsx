...
const AddFunds = () => {
    ...
    const { user, apiBaseUrl, updateCurrentUser } = useAuth();
    ...
    const handleAddFunds = async (event) => {
        ...
        try {
            const response = await fetch(`${apiBaseUrl}/customers/self-add-funds`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                setSeverity('success');
                updateCurrentUser(); // Update user context to reflect new funds
                setAmount('');
            } else {
